...
export function StockOperationsView() {
  const { products, transactions, adjustStock, canEditInventory } = useInventory()
  ...
  const handleExecuteOperation = (e: React.FormEvent) => {
    e.preventDefault()
    if (!selectedProductId || qty <= 0 || !canEditInventory) return
      ...
      <div className="p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-200/80 dark:border-slate-800 shadow-sm flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">
            Physical stock movements across warehouses and live audit logs.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-6 bg-white dark:bg-slate-900 p-6 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm space-y-4">
          <div className="flex items-center justify-between">
            ...
            {!canEditInventory && (
              <span className="text-[10px] font-semibold text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-950/50 px-2 py-0.5 rounded-lg">
                Requires Inventory Editor Role
              </span>
            )}
          </div>
          ...
          <form onSubmit={handleExecuteOperation} className="space-y-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              <button
                ...
                } ${!canEditInventory ? 'opacity-50 cursor-not-allowed' : ''}`}
              >
            ...
            >
              {canEditInventory ? 'Post Stock Entry' : 'View Only Mode'}
            </Button>
          </form>
        </div>
